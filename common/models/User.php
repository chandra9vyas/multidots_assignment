...
class User extends BaseModel implements IdentityInterface
{
	...
	public function rules()
	{
		return [
			...
			[['u_first_name', 'u_last_name','u_email', 'password'], 'required', 'on' =>'signup'], 
			[['u_email'], 'unique'], 
			[['u_email'], 'email'], 
			...
			[['u_first_name', 'u_insta_id', 'u_insta_token','u_username'], 'required', 'on' =>'instalogin'], 
			[['u_insta_id','u_username'], 'required', 'on' =>'instalogin'],
			// password validation
	...
	*/
	...
	{       
		...
		if($this->isNewRecord){			
			...
			$this->u_username = empty($this->u_username) ? $this->u_first_name.'_'.time() : $this->u_username;
			$this->u_created = dt();           
		}
	...
	public static function GetUsername($id=NULL)
	{
		...
		if(isset($userDetail['u_id'])){
			return ucwords($userDetail['u_first_name'].' '.$userDetail['u_last_name']);
		}
		return "--";
	}
	/**
	...
	*/
	public function validLogin($email=NULL,$instaId=NULL){ 
		$cond = [];
		$getUser = 1;
		$user = [];
		if(isset($email) && !empty($email)){
			...
			$cond['u_email'] = $email;
			$getUser = 1;
		}elseif(isset($instaId) && !empty($instaId)){

			$cond['u_insta_id'] = $instaId;
			$getUser = 1;
		}
		if($getUser){
			$user = User::find()
